...
import {BASE_URL, WHO_SECULARS_ARE} from '../services/Endpoints';

import {WebView} from 'react-native-webview';

const LayPeopleScreen = ({navigation}) => {
  const {containerLay, webViewContainer} = styles;
  ...
  const runFirst = `
        ...
        const imageEl = document.querySelectorAll('.thumbnail_right');

        imageEl.forEach(function(singleImage) {
            singleImage.style.margin =  '20px';
            singleImage.style.float =  'right';
        });

        document.querySelector('p:nth-child(5)').style.clear = 'both';
        document.querySelector('p:nth-child(7)').style.clear = 'both';
        document.querySelector('div:last-child').style.clear = 'both';
  ...
  useEffect(() => {
    (async () => {
      const endpoint = BASE_URL + WHO_SECULARS_ARE;
...
export default LayPeopleScreen;
